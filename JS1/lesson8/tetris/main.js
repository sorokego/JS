document.addEventListener('DOMContentLoaded', () => {
    ...
    //Next element
    var nextElementField  = document.getElementById('nextElement');

    var nextElementObject = {
        cells: [],
        currentFigure: null,
    ...
    function pauseHandler() {
        if (gamePaused) {
            ...
            nextElementObject.cells.forEach((cell) => cell.element.className = 'closed');
    ...
    function drawFigureNextFigureField() {
        const NEXTWIDTH = 4;

        for (var i = 0; i < 16; i++) {
            nextElementObject.cells[i].element.className = 'closed';
        }

        switch (gameBoard.nextFigureNumber) {
            case 0:
                nextElementObject.cells[1].element.className = gameBoard.nextColor;
                nextElementObject.cells[1+NEXTWIDTH].element.className = gameBoard.nextColor;
                nextElementObject.cells[1+NEXTWIDTH*2].element.className = gameBoard.nextColor;
                nextElementObject.cells[2+NEXTWIDTH*2].element.className = gameBoard.nextColor;
                break;
            case 1: 
                nextElementObject.cells[2].element.className = gameBoard.nextColor;
                nextElementObject.cells[2+NEXTWIDTH].element.className = gameBoard.nextColor;
                nextElementObject.cells[2+NEXTWIDTH*2].element.className = gameBoard.nextColor;
                nextElementObject.cells[1+NEXTWIDTH*2].element.className = gameBoard.nextColor;
                break
            case 2:
                ...
                nextElementObject.cells[2+NEXTWIDTH].element.className = gameBoard.nextColor;
                nextElementObject.cells[2+NEXTWIDTH*2].element.className = gameBoard.nextColor;
                nextElementObject.cells[3+NEXTWIDTH*2].element.className = gameBoard.nextColor;
                break;
            case 3:
                nextElementObject.cells[2+NEXTWIDTH].element.className = gameBoard.nextColor;
                nextElementObject.cells[3+NEXTWIDTH].element.className = gameBoard.nextColor;
                nextElementObject.cells[2+NEXTWIDTH*2].element.className = gameBoard.nextColor;
                nextElementObject.cells[1+NEXTWIDTH*2].element.className = gameBoard.nextColor;
                break;
            case 4:
            ...
            case 6:
                ...
                nextElementObject.cells[1+NEXTWIDTH].element.className = gameBoard.nextColor;
                nextElementObject.cells[1+NEXTWIDTH*2].element.className = gameBoard.nextColor;
                nextElementObject.cells[1+NEXTWIDTH*3].element.className = gameBoard.nextColor;
                break;
        }
    }
    ...
    function initNextField () {
        // Очищаем поле на странице
        nextElementField.innerHTML = '';

        // Создаем поле
        nextElementObject.cells = [];
        ...
        for (var i = 0; i < 16; i++) {
            ...
            nextElementField.appendChild(cell);

            nextElementObject.cells[i] = { element: cell, index: i, haveBlock: false, color: '' };
        } 
    }
